...
2. Run Release restore/build/test and `scripts/package.ps1 -Version X.Y.Z` on Windows. This automatically compiles the release into `artifacts/publish/vX.Y.Z/` and packages portable zips and installers. See [compiling and publishing](compiling-and-publishing.md) for full details.
...
For a non-interactive lifecycle/provider smoke test, run `.\artifacts\publish\vX.Y.Z\SysWatt.App.exe --smoke-test`. It performs live collection for 2.5 seconds and then follows the normal deterministic shutdown path.
