...
SysWatt first checks for sensor values published by a running HWiNFO instance. If HWiNFO Shared Memory Support is enabled, SysWatt consumes that read-only snapshot and skips concurrent LibreHardwareMonitor access for the cycle. Otherwise, it enables supported LibreHardwareMonitor categories, opens the provider lazily, and recursively visits every hardware and sub-hardware node. Each node is updated before its sensors are read. Raw records retain provider, hardware and sensor identifiers/names/types, unit, timestamp, availability, value, and error context.
...
6. provider priority, with HWiNFO shared memory preferred when published, then LibreHardwareMonitor, then targeted fallbacks;
...
- Multiple programs polling Ryzen SMU or Super-I/O registers can conflict. When HWiNFO is running, enable **Settings → Shared Memory Support** so SysWatt reads its published snapshot instead of opening a competing low-level session. SysWatt does not bundle, patch, or automate HWiNFO.
